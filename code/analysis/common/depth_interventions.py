"""Depth-intervention helpers for the block-3, block-6, and block-9 interventions
(data/claim2/depth_interventions/).
...
from .bootstrap import bootstrap_delta_ci
from .interpolate import interpolate_bracket
...
def degree_weight(ell: int) -> float:
    ...
    balanced / depth-intervention P_bal metric."""
    return 1.0 / (2 * ell + 1)
...
def denom_bal(ss_by_ell_raw: dict, ss_ell0_centered: float) -> float:
    ...
    the `denom_bal` field already stored in depth_interventions_summary.json's `curves` entries
...
def interpolate_curve_to_support(
    ...
    shared common/interpolate.py bracketing rule -- same convention as
    code/analysis/claim2/build_frontier_ell4_degree_balanced.py."""
...
def depth_contrast(delta_a: float, delta_b: float) -> dict:
    ...
    depth_interventions_summary.json's `depth_contrasts` naming (`{deep}_minus_{shallow}`)."""
