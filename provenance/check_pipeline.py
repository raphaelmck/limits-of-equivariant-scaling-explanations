...
Two levels, depending on what is available:
  ...
  its header names the original path and the original file's SHA256. This proves the released
  tree has not drifted since the manifest was written.

  When the source tree is available (`--source-root <path>`, repeatable): each original's SHA256
  matches `original_sha256`, and the released copy is shown to be logically identical to the
  original. That comparison is made on the abstract syntax tree, with docstrings removed and the
  release's renames undone, so it is insensitive to comments, docstrings, and formatting but
  sensitive to any change in code -- a renamed variable, a changed constant, a reordered call, an
  added or removed statement all fail.

The exact source revision (repository name, branch, commit hash) this manifest was built from is
withheld from the manifest and from every file's header during double-blind review -- see
provenance/README.md. That withholding does not weaken either check above: both compare file
content by hash and by AST, never by revision identifier.
...
def main() -> int:
    ...
    print("(source revision withheld from this manifest for double-blind review; "
