# code/experiments/ — the code that produced the frozen data
...
withheld from this public copy for double-blind review and is recorded privately for restoration
after acceptance. Six files under `evaluation_pools/` were copied from a working directory that
was not under version control at the time; the copies here are now the versioned record of that
code.
...
| `run_depth_interventions.py` | the same intervention at blocks 3 and 6 |
