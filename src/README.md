...
data or numpy arrays already loaded by a caller in `scripts/`.

- **`io_utils.py`** — CSV/JSON/JSONL helpers, and `data_path()`, `manifest_path()`, `out_path()`:
  the only sanctioned way to build a path into `data/`, `manifests/`, or `analysis_out/`.
...
- **`force_scaling_fit.py`** — frontier construction and the power-law fit behind the scaling
  exponents.
- **`ood_analysis.py`** — re-aggregation of the intervention runs on the two evaluation
...
- **`seed_replication.py`** — the independent-run replication, which uses the raw total
  ...
  `ood_analysis.py` so the two metrics cannot be conflated.
- **`depth_localization.py`** — degree-balanced helpers for the block-3, block-6, and block-9
  comparison.
