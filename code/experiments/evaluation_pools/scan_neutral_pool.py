#!/usr/bin/env python3
# Adapted for anonymous release from esen_irrep_ood_2026_08_22/id_pool/scan_id_pool.py
# source revision unversioned at port time, original SHA256 5896e87ecbfdb3fd0cd1bde9cff8f1414645706baeec5dc58437ddbea8af1c08
...
# Logic unchanged; import paths, filesystem paths, and identifying names rewritten.
# Verify with: python3 provenance/check_pipeline.py
"""Freeze the Neutral evaluation population, using the identical eligibility criteria and nested
stratified-pool procedure as the support-matched pool, pointed at the Neutral validation
split the eSEN checkpoints were trained against.

Phase 2: scan the full neutral_val split and flag structures with charge==0, spin==1,
          and all Z in the Neutral training support (same support set as OOD).
Phase 3: freeze a stratified, nested ID pool (1024/2048/4096/8192/16384), same seed.
Phase 4: write audit/report/manifest artifacts (ID-pool naming).
"""
...
VAL_SRC = "<DATA_ROOT>/open_mol/neutral_val"
OUT_DIR = "<PROJECT_ROOT>/analysis_outputs/esen_irrep_ood_2026_08_22/id_pool"
DATASET_DIR = "<PROJECT_ROOT>/analysis_outputs/esen_irrep_ood_2026_08_22/dataset"
SEED = 42
...
def main():
    ...
    print("=== PHASE 1: reuse frozen neutral-train element support ===", flush=True)
    with open(os.path.join(DATASET_DIR, "neutral_element_support.json")) as f:
        p1 = json.load(f)
    print(f"[phase1] loaded support ({len(p1['support'])}): {p1['support']}", flush=True)

    print("=== PHASE 2: neutral_val eligibility scan ===", flush=True)
    ...
    print("=== PHASE 3: stratified nested ID pool ===", flush=True)
    p3 = phase3_stratified_pool(p2)

    np.save(os.path.join(OUT_DIR, "id_pool_indices.npy"), p3["pool_global_idx"])
    ...
    with open(os.path.join(OUT_DIR, "id_pool_manifest.json"), "w") as f:
    ...
    lines.append("\n## 2. neutral_val eligibility scan\n")
    ...
    lines.append("\nOutputs in this directory: `eligible_summary.json`, "
                  "`eligible_full_index.npz` (full eligible table), `id_pool_manifest.json`, `id_pool_indices.npy`, `subset_summary.csv`.\n")
