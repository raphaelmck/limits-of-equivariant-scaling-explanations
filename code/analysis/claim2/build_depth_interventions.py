...
Raw inputs, all in data/claim2/: depth_interventions/depth_interventions_M1024.jsonl for blocks 3
...
from common.depth_interventions import degree_weight, denom_bal, p_bal
...
BOOT_SEED = 20260820  # documented (same seed used across this whole line of work); bit-exact
                        # against depth_interventions_summary.json -- see module docstring.
N_CFG = 1024
...
def load_block36():
    rows = read_jsonl(data_path("claim2", "depth_interventions", "depth_interventions_M1024.jsonl"))
    block_stats = read_json(data_path("claim2", "depth_interventions", "block_stats_M1024.json"))
    by_owner_layer = {}
    for r in rows:
...
def main():
    result = build()
    out = out_path("claim2", "depth_interventions_recomputed.json")
