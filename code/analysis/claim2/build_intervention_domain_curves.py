...
per-configuration arrays in data/claim2/per_config_intervention_raw/.

Writes into analysis_out/claim2/:
  alpha_curves_neutral_and_support_matched.csv        damage curve per checkpoint, population, and alpha
  high_low_contrast_pool_size_convergence.csv          high-minus-low contrast as a function of pool size
  domain_interaction_high_low_contrast.csv   the same contrast across populations
  matched_compute_lmax2_vs_lmax4_baseline.csv  matched-compute log(L_2 / L_4), the source of Figure 2B

The contrasts are taken at the common P_bal support edge (common/domain_analysis.interp_at_x), not at
...
from common.io_utils import data_path, out_path, read_json, read_jsonl, write_csv
from common.domain_analysis import (
    ALPHAS,
    BOOT_SEED_ANALYZE,
    N_BOOT,
    OWNER_ORDER,
...
RAW = lambda *p: data_path("claim2", "per_config_intervention_raw", *p)
...
def main():
    ...
    write_csv(out_path("claim2", "alpha_curves_neutral_and_support_matched.csv"), full_curve_rows)
    write_csv(out_path("claim2", "matched_compute_lmax2_vs_lmax4_baseline.csv"), h_rows)
    ...
    write_csv(out_path("claim2", "high_low_contrast_pool_size_convergence.csv"), g_rows)
    ...
    write_csv(out_path("claim2", "domain_interaction_high_low_contrast.csv"), interaction_rows)

    print("wrote alpha_curves_neutral_and_support_matched.csv, high_low_contrast_pool_size_convergence.csv, "
          "domain_interaction_high_low_contrast.csv, matched_compute_lmax2_vs_lmax4_baseline.csv")
