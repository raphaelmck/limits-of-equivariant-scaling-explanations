...
def validate_dense_grid_ranking_comparison():
    ...
    code/analysis/claim1/build_dense_grid_ranking_comparison.py."""
...
def validate_dense_grid_pairwise_gap():
    ...
    budgets in fixed order. See code/analysis/claim1/build_dense_grid_pairwise_gap.py."""
...
def validate_frontier_ell4_degree_balanced():
    ...
    see code/analysis/claim2/build_frontier_ell4_degree_balanced.py. Now validates per-alpha CIs AND
...
def validate_seed_replication_result():
    ...
    from analyze_seed_replication.py -- see code/analysis/claim2/build_seed_replication_result.py.
...
def validate_depth_interventions():
    """BIT-EXACT for all 3 depths (block 3, 6, and 9) and all 12 depth
    contrasts (block9-involving contrasts included) -- see
    code/analysis/claim2/build_depth_interventions.py, which ports the upstream depth-intervention report script's
    single shared np.random.default_rng(20260820) idx matrix across every owner and depth."""
    name = "claim2/depth_interventions_summary.json (delta + CI BIT-EXACT for all 3 depths + 12 contrasts)"
    regen = read_json(out_path("claim2", "depth_interventions_recomputed.json"))
    frozen = read_json(data_path("claim2", "depth_interventions", "depth_interventions_summary.json"))
    ok = True
    details = []
    ...
    record("claim2/depth_interventions_recomputed.json (low_vs_high D_k, self-consistency, no upstream frozen target)",
...
def validate_intervention_domain_curves():
    ok_all = True
    for fname, keys, pt, ci in [
        ("alpha_curves_neutral_and_support_matched.csv", ["tag", "domain", "M", "alpha"], "delta", ("delta_ci_lo", "delta_ci_hi")),
        ("high_low_contrast_pool_size_convergence.csv", ["tag", "M"], "G", ("G_ci_lo", "G_ci_hi")),
        ("domain_interaction_high_low_contrast.csv", ["M"], "interaction", ("ci_lo", "ci_hi")),
        ("matched_compute_lmax2_vs_lmax4_baseline.csv", ["lmax4_tag", "domain", "M"], "H_force", ("H_ci_lo", "H_ci_hi")),
    ]:
...
def validate_chemistry_domain_decomposition():
    """BIT-EXACT: np.random.default_rng(20260823), the per-configuration
    data_id chemistry-family label recovered from data/claim2/chemistry_domain_labels.json -- see
    code/analysis/claim2/build_chemistry_domain_decomposition.py."""
    ok_all = True
    for fname, keys, pt, ci in [
        ("chemistry_domain_decomposition.csv", ["stratum", "domain", "tag"], "delta", ("delta_ci_lo", "delta_ci_hi")),
        ("chemistry_family_positive_control.csv", ["stratum"], "contrast_w24_minus_w10", ("ci_lo", "ci_hi")),
    ]:
        regen = read_csv(out_path("claim2", fname))
        frozen = read_csv(data_path("claim2", fname))
        fmap = {tuple(r[k] for k in keys): r for r in frozen}
...
def main():
    ...
    c2_depth = validate_depth_interventions()
    record_main("C2. depth-localization result", c2_depth)

    c2_ood_curves = validate_intervention_domain_curves()
    c2_ood_domain = validate_chemistry_domain_decomposition()
    ...
    # already checked (matched_compute_lmax2_vs_lmax4_baseline.csv); re-derive its pass/fail standalone label.
    regen_h = read_csv(out_path("claim2", "matched_compute_lmax2_vs_lmax4_baseline.csv"))
    frozen_h = read_csv(data_path("claim2", "matched_compute_lmax2_vs_lmax4_baseline.csv"))
