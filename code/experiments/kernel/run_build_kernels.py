...
UNCHANGED code paths:
  ...
  1. Owner loading / checkpoint provenance / strict state-dict load / config snapshot, imported
     ...
     built `frontier_checkpoint_manifest.csv` in the first place.
...
global_step provenance check against data/claim1/frontier_checkpoint_manifest.csv before touching the
...
OWNER_MANIFEST_CSV = Path(
    "data/claim1/frontier_checkpoint_manifest.csv"
)
...
def check_owner_provenance(owner: OwnerSpec) -> dict[str, Any]:
    """Mandatory checkpoint-provenance validation against frontier_checkpoint_manifest.csv, BEFORE any GPU
