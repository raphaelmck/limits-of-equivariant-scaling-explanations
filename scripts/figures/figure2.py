...
own training compute, joined from the checkpoint table.
"""
...
def lmax4_compute():
    ...
    rows = read_csv(os.path.join(REPO_ROOT, "data", "claim2", "matched_compute_lmax_owners.csv"))
...
def _dense_run_keys():
    ...
    dep = read_csv(os.path.join(ANALYSIS_OUT, "lmax24_frontier_dependence.csv"))
    return {(r["width"], r["step"]) for r in dep if r["run_name"] == DENSE_RUN_NAME}


def panel_b(ax, causal_rows):
    gaps = read_csv(os.path.join(ANALYSIS_OUT, "lmax24_all_frontier_gaps.csv"))
...
def main():
    ...
    causal_rows = read_csv(os.path.join(ANALYSIS_OUT, "lmax4_causal_vs_comparative.csv"))
