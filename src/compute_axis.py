...
from .io_utils import data_path, manifest_path, read_csv
...
def budget_flops():
    ...
    out = {}
    for row in read_csv(manifest_path("checkpoints.csv")):
        if row["claim"] != "1":
            continue
    ...
    for row in read_csv(data_path("claim1", "dense_grid_owner_force_mse.csv")):
